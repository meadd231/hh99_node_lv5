...
describe('Layered Architecture Pattern, Posts Domain Integration Test', () => {
  beforeAll(async () => {
    const res = await request(app)
      .post('/login')
      .send({ username: 'test', password: 'qwer1234' });
  ...
  test('POST /signup API 회원가입', async () => {
    const response = await supertest(app)
  ...
  test('POST /posts API (createPost) Integration Test Success Case', async () => {
    // POST /posts API에서 사용되는 body 데이터입니다.
    ...
    const response = await supertest(app)
      .post(`/posts`) // API의 HTTP Method & URL
      .query({}) // Request Query String
      .set('Cookie', cookie)
    ...
    // 1. API를 호출하였을 때, 성공적으로 실행할 경우 201 Http Status Code를 반환한다.
    expect(response.json).toEqual(201);
    expect(response.status).toEqual(201);
  ...
  // test('POST /posts API (createPost) Integration Test Error Case, Invalid Params Error', async () => {
  //   // POST /posts API에서 에러를 발생시키기 위해 사용되는 body 데이터입니다.
  ...
  test('GET /posts API (getPosts) Integration Test Success Case, is Exist Posts Data', async () => {
    // POST /posts API에서 사용되는 body 데이터입니다.
    const createPostRequestBodyParams = {
